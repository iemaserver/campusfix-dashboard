...
def dashboard_stats():
    ...
    in_progress = complaints_collection.count_documents({**query, "status": "In Progress"})
    ...
    last_month_count = complaints_collection.count_documents({
        **query,
        "created_at": {"$gte": start_last_month, "$lt": start_this_month}
    })

    if last_month_count > 0:
        month_change = round(((this_month_count - last_month_count) / last_month_count) * 100)
